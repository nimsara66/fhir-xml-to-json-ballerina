...
This Ballerina library provides functionality to convert FHIR (Fast Healthcare Interoperability Resources) XML to JSON and Record formats. It utilizes a Java methods for efficient FHIR data conversion.

## Features

- Convert FHIR XML to JSON format.
- Convert FHIR XML to Ballerina Record format.
...
public function main() returns error? {
    ...
    </Patient>`; // FHIR XML input

    json patientInJson = check fhir:fhirXmlToJson(patient);
    io:println(patientInJson);

    international401:Patient patientWithType = check fhir:fhirXmlToRecord(patient, international401:Patient);
    io:println(patientWithType);
}
